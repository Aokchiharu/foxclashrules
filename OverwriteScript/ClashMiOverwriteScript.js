...
// 目标：Clash Mi 1.0.29 系列 / Mihomo 1.19.30；绑定原始节点订阅。
// 在应用中启用 TUN 覆写；追加覆写选择「内置-覆写」。
// 修正版：main 直接返回配置对象，适配 iOS 的 Map 返回值要求。
const OPTIONS = {
...
function applyPurposeConfig(config) {
  ...
  const regions = [["JP","🇯🇵|日本|Japan|\\bJP(?=\\d|\\b)"],["TW","🇹🇼|台湾|台灣|Taiwan|\\bTW(?=\\d|\\b)"],["SG","🇸🇬|新加坡|狮城|獅城|Singapore|\\bSG(?=\\d|\\b)"],["HK","🇭🇰|香港|Hong\\s*Kong|\\bHK(?=\\d|\\b)"],["US","🇺🇸|美国|美國|United\\s*States|\\bUSA?\\b|\\bUS(?=\\d)"],["EU","🇳🇱|🇬🇧|🇩🇪|🇫🇷|🇨🇭|🇸🇪|🇫🇮|🇳🇴|🇩🇰|🇮🇹|🇪🇸|🇵🇱|🇦🇹|🇮🇪|🇵🇹|荷兰|荷蘭|英国|英國|德国|德國|法国|法國|瑞士|瑞典|芬兰|挪威|丹麦|意大利|西班牙|波兰|奥地利|爱尔兰|葡萄牙|Netherlands|United\\s*Kingdom|Britain|Germany|France|Switzerland|Sweden|Finland|Norway|Denmark|Italy|Spain|Poland|Austria|Ireland|Portugal|\\b(?:NL|UK|GB|DE|FR|CH|SE|FI|NO|DK|IT|ES|PL|AT|IE|PT)(?=\\d|\\b)"],["AS","🇰🇷|🇲🇾|🇮🇳|🇹🇭|🇻🇳|🇮🇩|🇵🇭|🇲🇴|🇨🇳|韩国|韓國|马来西亚|馬來西亞|印度|泰国|泰國|越南|印尼|菲律宾|菲律賓|澳门|澳門|中国|中國|Korea|Malaysia|India|Thailand|Vietnam|Indonesia|Philippines|Macau|Macao|China|\\b(?:KR|MY|IN|TH|VN|ID|PH|MO|CN)(?=\\d|\\b)"]].map(function(item) { return [item[0], new RegExp(item[1], "i")]; });
...
function main(config) {
  ...
  // Mihomo 已移除全局指纹；保留订阅节点自身的 client-fingerprint。
  ...
  // iOS 的 JS 桥接将返回值转换为 Map，因此必须直接返回对象。
  return config;
}
